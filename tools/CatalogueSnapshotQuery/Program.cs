...
//
// CI is a separate problem: it never has the file, so it cannot run any of this. What it can run
// is a sample — a few hundred catalogue foods with their nutrients and their letters written down,
// small enough to commit. --sample writes that fixture; CatalogueSampleTests reads it. The gate's
// 68 foods stay what they are, a hand-picked argument; the sample is the catalogue's own shape.
//
...
//   dotnet run -- --sample  regenerates the CI fixture from the catalogue
...
// One food in 14, taken at a fixed stride down the ids, plus a couple from every category that
// carries a rule. The stride keeps the sample proportional to the catalogue without choosing
// which foods are convenient; the categories are added because a dispatch table nothing exercises
// is the failure CategoryCheckQuery exists to catch, and CI should catch it too.
const int SampleStride = 14;
const int PerRuledCategory = 2;
...
var inputs = new Dictionary<int, FoodInput>();
var incomplete = 0;
...
void WriteSample()
{
    const string path = "../../server/Sated.Scoring.Tests/catalogue-sample.csv";

    var ids = current.Keys.Where((_, index) => index % SampleStride == 0).ToHashSet();

    foreach (var category in shipped.Rules.All.Select(rule => rule.Category).Distinct(
        StringComparer.OrdinalIgnoreCase))
    {
        foreach (var id in current.Keys
            .Where(id => inputs[id].Category.Equals(category, StringComparison.OrdinalIgnoreCase))
            .Take(PerRuledCategory))
        {
            ids.Add(id);
        }
    }
    ...
    file.AppendLine("# Generated by tools/CatalogueSnapshotQuery --sample. Do not edit by hand.");
    file.AppendLine("# A slice of FNDDS with the letters this engine gives it, so CI can test the");
    file.AppendLine("# catalogue: the catalogue itself is 63 MB and cannot live in the repository.");
    file.AppendLine($"# Every {SampleStride}th food by id, plus {PerRuledCategory} from each ruled category.");
    file.AppendLine($"# Calibration: {shipped.Catalogue}, measured {shipped.MeasuredOn}");
    file.AppendLine("# Values are per 100 g, in the units of DensityInput. Description is the rest of the line.");

    file.AppendLine(string.Join(',',
        ["FdcId", "Calories", "Protein", "Fat", "Fiber", "VitaminA", "VitaminC", "VitaminE",
         "Calcium", "Iron", "Magnesium", "Potassium", "SaturatedFat", "Sodium", "VitaminD",
         "Thiamine", .. lenses.Select(lens => lens.Name), "Category", "Description"]));
